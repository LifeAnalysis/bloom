...
**bloom-eth presents Ethereum (and a growing set of EVM L2s) as a virtual
...
- A single Rust daemon (`bloom-eth` / `beth`) exposing a virtual
...
- CLI: `beth init | up | wallet {new,import,list} | watch | …`.
...
- Distributed peer sync (this is the bloom MVP feature; bloom-eth is
...
```
   ...
   /eth          └────────────────┬─────────────────────┘
       ...
       chains/  wallets/  defi/   ens/   prices/  watch/  …  (handlers)
        ...
        ┌────────────────────────────────────────────────┐
        │              Storage / Cache Layer             │
        │   ~/.bloom-eth/{config,keystore,cache,logs}/   │
...
**[OPEN-A]** Should bloom-eth re-use bloom's NFS substrate (a
...
2. SQLite (`~/.bloom-eth/cache.db`) for structured data (txs, blocks,
   activity pages, ABIs).
3. Blob directory (`~/.bloom-eth/blobs/`) for source code archives,
   large logs.
...
- Endpoint config per chain in `chains/<chain>/rpc.toml`, editable
  ...
  - **[OPEN-C]** Editing chain config via the FS is conceptually neat
    ...
    config lives outside the mount in `~/.bloom-eth/config/`.
...
- `beth wallet unlock <name>` prompts passphrase; key in memory until
  daemon stop or `beth wallet lock`.
...
- For lower latency, an out-of-band Unix socket
  (`~/.bloom-eth/events.sock`) streams the same events as JSON Lines
...
```
beth init                          # config + first wallet, prompt passphrase
beth up                            # start daemon, mount /eth
beth down                          # graceful stop, unmount
beth status                        # mirror of /eth/status/daemon.json
beth wallet new <name>             # generate or import
beth wallet import <name> <key|file|mnemonic>
beth wallet list
beth wallet unlock <name>
beth wallet lock <name>
beth wallet export <name>          # encrypted only; refuses plaintext
beth send <wallet> <amount> <to>   # convenience over outbox
beth watch <spec>                  # convenience over watch/new
beth chain add <name> --rpc <url> --etherscan <url>  # custom chain
beth doctor                        # prints diagnostics, RPC health
beth tail <path>                   # follows a /eth path
beth whois <addr>                  # resolves ENS / contract name
```

The CLI is a thin wrapper over the FS; everything `beth` does is also
...
the key into bloom-eth, stage and confirm:
...
crates/
├── beth          # CLI binary
├── beth-daemon   # daemon binary; depends on the rest
├── beth-vfs      # path router + handler trait + caching
├── beth-chain    # rpc pool, chain engine, indexer
├── beth-tx       # tx engine, intent parser, policy
├── beth-keystore # signer + key encryption
├── beth-defi     # Enso client + DeFi abstractions
├── beth-watch    # subscriptions
├── beth-mount    # NFS adapter (likely bloom-mount once factored out)
...
- **[OPEN-E]** Per-chain RPC config UX — config file vs. `beth chain
...
- **[OPEN-J]** Naming. Is `beth` (the binary) too cute / confusable
...
3. **Forking sandbox.** `simulate/<session>/` could optionally back
   ...
   later.
4. **DSL for batched intents.** A `.beth` file format that the agent
