...
mod tests {
    ...
    fn signed_tempo_aa_raw_tx_with_nonce(fee_payer_signed: bool, nonce: u64) -> String {
        type TestSigner = alloy_signer_local::LocalSigner<k256::ecdsa::SigningKey>;
        let user: TestSigner = USER_PK.parse().unwrap();
        let fee_payer: TestSigner = FEE_PAYER_PK.parse().unwrap();
        ...
        let user_sig = futures::executor::block_on(alloy_signer::Signer::sign_hash(
            &user,
            &tx.signature_hash(),
        ))
        .unwrap();
        if fee_payer_signed {
            tx.fee_payer_signature = Some(
                futures::executor::block_on(alloy_signer::Signer::sign_hash(
                    &fee_payer,
                    &tx.fee_payer_signature_hash(user.address()),
                ))
                .unwrap(),
            );
        }
