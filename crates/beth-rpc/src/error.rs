...
/// Top-level error variant returned by `beth-rpc` constructors and the
/// (still-stub) session API. Callers in `beth-chain` translate this into
/// their own `ChainError` so we don't leak this enum across crate
/// boundaries unnecessarily.
#[derive(Debug, Error)]
pub enum BethRpcError {
