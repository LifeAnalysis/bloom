...
//! avoid a circular dep with `beth-chain` — that crate's
...
///
/// Construct with `beth_chain::ChainClient::open_session`. While the
...
impl<'a> Session<'a> {
    /// Internal constructor used by `beth-chain`'s `open_session`. The
    ...
    pub async fn balance(&self, addr: Address) -> Result<U256, BethRpcError> {
    ...
    pub async fn nonce(&self, addr: Address) -> Result<u64, BethRpcError> {
        self.run_with_pin(|block| self.provider.get_transaction_count(addr).block_id(block))
            .await
    }

    /// `eth_getCode(addr, pinned)`.
    pub async fn code(&self, addr: Address) -> Result<Vec<u8>, BethRpcError> {
        let bytes = self
    ...
    /// `eth_call(req, pinned)`.
    pub async fn eth_call(&self, req: TransactionRequest) -> Result<Bytes, BethRpcError> {
    ...
    pub async fn get_storage_at(&self, addr: Address, slot: U256) -> Result<B256, BethRpcError> {
        let val: U256 = self
    ...
    async fn run_with_pin<F, Fut, T>(&self, build: F) -> Result<T, BethRpcError>
    where
        F: Fn(BlockId) -> Fut,
