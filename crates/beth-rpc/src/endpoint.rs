...
///
/// Returns `BethRpcError::InvalidUrl` for unparseable strings and for
...
/// `Unknown` variant.
pub fn classify_endpoint(spec: &EndpointSpec) -> Result<(Url, EndpointScheme), BethRpcError> {
    let url = Url::parse(&spec.url).map_err(|e| BethRpcError::InvalidUrl {
        url: spec.url.clone(),
        source: e,
    })?;
    let scheme = match url.scheme() {
        ...
        other => {
            return Err(BethRpcError::InvalidUrl {
                url: spec.url.clone(),
