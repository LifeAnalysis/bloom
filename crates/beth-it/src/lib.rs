//! Shared helpers for `beth-it`'s integration tests.
//!
...
/// Foundry binaries; rely on `$PATH`. Override with `BETH_ANVIL_BIN` /
/// `BETH_CAST_BIN` if you need to point at a specific install.
pub fn anvil_bin() -> String {
    std::env::var("BETH_ANVIL_BIN").unwrap_or_else(|_| "anvil".to_string())
}

pub fn cast_bin() -> String {
    std::env::var("BETH_CAST_BIN").unwrap_or_else(|_| "cast".to_string())
}
