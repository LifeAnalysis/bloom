...
//! its coverage lives in the `beth-revert` unit tests.
//!
...
use beth_revert::{
    boxed, BuiltinDecoder, BytecodeSource, DecodeContext, DecodeSource, DecoderChain,
    HeimdallDecompileDecoder,
};
    ...
    let _ = tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env().unwrap_or_else(|_| {
                tracing_subscriber::EnvFilter::new("warn,beth_revert=info,beth_chain=info")
            }),
        )
        .with_test_writer()
        .try_init();
