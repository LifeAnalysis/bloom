...
//!
//! Gated `#[ignore]` to match the rest of `beth-it`. Run with:
//!
//! ```text
//! cargo test -p beth-it -- --ignored rpc_ws_subscriptions
//! ```
//!
//! Skips if `anvil` is not on `$PATH` (the spawn helper times out and
//! the test fails — same convention as the other beth-it tests).
    ...
    let _ = tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env().unwrap_or_else(|_| {
                tracing_subscriber::EnvFilter::new("warn,beth_rpc=debug,beth_watch=debug")
            }),
        )
        .with_test_writer()
        .try_init();
