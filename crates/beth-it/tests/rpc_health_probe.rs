...
//!
//! Like the rest of `beth-it`, the test is gated `#[ignore]` so CI
//! runs that lack a foundry install (or just don't want to spawn
//! processes) skip cleanly. Invoke with:
//!
//! ```text
//! cargo test -p beth-it -- --ignored rpc_health_probe
//! ```
    ...
    let _ = tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| tracing_subscriber::EnvFilter::new("warn,beth_rpc=debug")),
        )
        .with_test_writer()
        .try_init();
