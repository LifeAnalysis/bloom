//! Integration test for the WS-to-poll handover in `beth-watch`.
//!
//! Scenario:
//!
...
//!
//! Gated `#[ignore]` like the rest of `beth-it`. The handover is
...
use beth_watch::executor::WatchExecutor;
use beth_watch::{WatchKind, WatchRegistry, WatchSpec};
...
async fn block_watch_falls_back_to_poll_when_anvil_dies() -> Result<()> {
    let _ = tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env().unwrap_or_else(|_| {
                tracing_subscriber::EnvFilter::new("warn,beth_watch=debug,beth_rpc=debug")
            }),
        )
        .with_test_writer()
        .try_init();
    ...
    // when the test is run with `RUST_LOG=beth_watch=debug`); the poll
