...
//!
//! ```text
//! cargo test -p beth-watch -- --ignored
//! ```
//!
//! Requires Foundry's `anvil` and `cast` to be available at
//! `~/.foundry/bin/{anvil,cast}` (matching the path used by `beth-it`).
    ...
    let _ = tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| tracing_subscriber::EnvFilter::new("warn,beth_watch=info")),
        )
        .with_test_writer()
        .try_init();
