...
//!
//! The on-disk socket lives at `<home>/run/beth.sock` (mode 0600). Clients
...
/// Default socket path under `<home>/run/beth.sock`.
pub fn default_socket_path(home_root: &Path) -> PathBuf {
    home_root.join("run").join("beth.sock")
}
...
mod tests {
    ...
    async fn end_to_end_over_uds() {
        ...
        let sock = dir.path().join("beth.sock");
