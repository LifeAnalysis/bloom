...
use crate::{ProtocolError, ProtocolErrorCode, SignedEnvelope};

#[derive(Clone, Copy, Debug, Deserialize, Eq, Hash, PartialEq, Serialize)]
pub enum ServiceRole {
    #[serde(rename = "bloom-machine")]
    Machine,
    #[serde(rename = "bloom-broker")]
    Broker,
    #[serde(rename = "bloom-signer")]
    Signer,
}
...
method_enum!(MachineBrokerMethod {
    ...
    BrokerCapabilities => "broker.capabilities",
    ActionValidate => "action.validate",
    SealedApprovalPrepare => "sealed_approval.prepare",
    SealedApprovalStatus => "sealed_approval.status",
    SealedApprovalList => "sealed_approval.list",
    SealedApprovalLimitState => "sealed_approval.limit_state",
    SealedApprovalRevoke => "sealed_approval.revoke",
    SealedApprovalRevokeAll => "sealed_approval.revoke_all",
    SealedApprovalRenew => "sealed_approval.renew",
    SigningSign => "signing.sign",
    SigningSignBatch => "signing.sign_batch",
    OperationStatus => "operation.status",
    OperationCancel => "operation.cancel",
    PolicyRead => "policy.read",
    PolicyValidateUpdate => "policy.validate_update",
    PolicyCommitUpdate => "policy.commit_update",
    WalletListPublic => "wallet.list_public",
    WalletGetPublic => "wallet.get_public",
...
/// Object-safe transport-independent role seam used by service clients.
pub trait RpcService: Send + Sync {
    fn call<'a>(
        &'a self,
        request: SignedEnvelope<serde_json::Value>,
    ) -> ServiceFuture<'a, SignedEnvelope<serde_json::Value>>;
}
...
mod tests {
    ...
    fn unknown_methods_fail_closed() {
        assert_eq!(
            MachineBrokerMethod::parse("signer.sign").unwrap_err().code,
            ProtocolErrorCode::UnknownMethod
        );
        assert_eq!(
