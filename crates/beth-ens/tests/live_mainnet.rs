...
//!
//! Skipped unless `BETH_MAINNET_RPC` is set. Also `#[ignore]`d so it
...
async fn vitalik_eth_round_trip() {
    let url = match std::env::var("BETH_MAINNET_RPC") {
        Ok(u) if !u.is_empty() => u,
        _ => {
            eprintln!(
                "skipping live ENS test: BETH_MAINNET_RPC not set. \
                 Set it to a mainnet HTTPS RPC URL (or a fork URL) and \
                 run with `cargo test -p beth-ens -- --ignored`."
            );
            return;
        }
    };
