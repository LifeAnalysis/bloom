//! The `PetalHost` bridge: a Petal's `bloom:chain/read` calls land on a
//! [`Mediator`], so every driver RPC is profile-mediated, allowlisted,
//! genesis-bound, and audited. Every other host authority is denied by
//! [`DenyHost`] — a chain-driver Petal declares no other capability through
//! this bridge; other host services (signing, storage) are wired
//! separately by Machine's main Petal host.
//!
//! This type carries no chain-specific logic: it was originally written for
//! the Solana driver and needed zero changes to serve any chain family — the
//! genericity already lived in [`Mediator`] itself.
