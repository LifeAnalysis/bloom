...
mod tests {
    ...
    /// `BETH_ETHERSCAN_KEY` in env. Skipped by default; run with
    /// `cargo test -p beth-etherscan -- --ignored`.
    #[tokio::test]
    #[ignore]
    async fn live_etherscan_smoke() {
        let key = match std::env::var("BETH_ETHERSCAN_KEY") {
            Ok(k) if !k.is_empty() => k,
            _ => {
                eprintln!("SKIP: BETH_ETHERSCAN_KEY not set");
                return;
            }
        };
