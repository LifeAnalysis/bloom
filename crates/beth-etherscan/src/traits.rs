...
//!
//! Both keep the existing `beth-etherscan` DTOs (`ContractSource`,
...
//!
//! Lives in `beth-etherscan` because:
//! - the crate already owns the response types and the `async-trait` dep,
//! - `beth-vfs` already depends on `beth-etherscan`,
//! - putting it in `beth-proto` would force `beth-proto` (a small,
