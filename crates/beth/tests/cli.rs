//! CLI smoke tests for the `beth` binary.
//!
//! Each test allocates a fresh `tempfile::tempdir()` home and invokes the
//! built `beth` binary via `assert_cmd::Command::cargo_bin`. We exercise
...
/// Build a Command for the `beth` binary that always runs against a
...
fn beth_cmd(home: &Path) -> Command {
    let mut cmd = Command::cargo_bin("beth").expect("locate beth binary");
    cmd.env("BETH_HOME", home);
...
fn wallet_new_via_env_passphrase() {
    // BETH_PASSPHRASE feeds the same arg via env. Confirms the env path
    // works and that the wallet ends up in the keystore directory on disk.
    let home = fresh_home();
    beth_cmd(home.path())
        .args(["wallet", "new", "bob"])
        .env("BETH_PASSPHRASE", "env-pass-1")
        .assert()
        .success()
