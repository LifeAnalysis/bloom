...
paths to credentials, never a credential or authenticator seed. The default is
under `XDG_STATE_HOME` (or `~/.local/state`), outside this repository.
...
stored. Do not edit this file by hand; rerun `init` only when status reports no
pending recovery.
...
Use `--state /private/path/to/state.json` on every operator subcommand to choose
a non-default state location. `BLOOM_EVAL_BLOOM_MOUNT` is never hard-coded by
the operator; all host paths are derived from the state initialized above.
